...
export async function checkDuplicateJobWorker(tenantId: string, extraction: JobExtractionResult): Promise<DuplicateCheckResult> {
...
export async function findSimilarApplication(input: {
  tenantId: string;
  extraction: JobExtractionResult;
  excludeJobId: string;
}): Promise<{ applicationId: string; company: string; position: string } | null> {
  const apps = await ApplicationModel.find({ tenantId: input.tenantId }).select("_id company position jobId").lean();
  for (const app of apps) {
    if (app.jobId && String(app.jobId) === input.excludeJobId) continue;
    if (
      normalizeText(app.company) === normalizeText(input.extraction.company) &&
      isSimilarPosition(app.position, input.extraction.position)
    ) {
      return { applicationId: String(app._id), company: app.company, position: app.position };
    }
  }
  return null;
}
