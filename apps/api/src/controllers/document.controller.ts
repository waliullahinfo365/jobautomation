...
export const exportPdf = asyncHandler(async (req: Request, res) => { const tenantId=assertTenantId(req.tenantId); const documentId=req.params.id; const userId=req.user?.id ?? "system"; const execute=req.query.execute==="true"; if(execute){ if(process.env.NODE_ENV==="production") throw new ApiError("Direct execute=true is disabled in production",403,"FORBIDDEN"); const result=await exportDocumentPdf({ tenantId, documentId, userId }); await logApiAction({tenantId,moduleKey:'pdf-export',status:'Success',message:'PDF export completed',relatedRecordType:'Document',relatedRecordId:req.params.id,metadata:{operationId:result.operationId,pdfUrl:result.pdfUrl}}); return successResponse(res,result,'PDF export completed'); } const queued=await enqueueAutomationModule({ tenantId, userId, moduleKey:"pdf-export", payload:{ documentId }, source:"api" }); return successResponse(res,{operationId:queued.operationId,jobId:queued.jobId,moduleKey:"pdf-export",status:queued.status,message:queued.message},'PDF export queued'); });
