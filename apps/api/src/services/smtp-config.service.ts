...
/** Loads decrypted SMTP credentials for outbound mail (tenant integration row). */
export async function getSmtpOutboundCredentials(tenantId: string): Promise<SmtpOutboundCredentials | null> {
  ...
  let pass: string;
  try {
    pass = decryptSecret(conn.accessTokenEncrypted);
  } catch {
    return null;
  }
