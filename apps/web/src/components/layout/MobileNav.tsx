    ...
    <>
      <header className="flex min-h-[52px] items-center justify-between border-b border-[var(--border-subtle)] bg-[rgba(11,13,18,0.85)] px-3 py-2.5 pt-[max(0.5rem,env(safe-area-inset-top))] backdrop-blur-md sm:min-h-[56px] sm:px-4 md:hidden">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[var(--text-1)]">{APP_NAME}</span>
        </div>
        <button
          type="button"
          onClick={() => setOpen(true)}
          className="grid h-9 w-9 place-items-center rounded-md text-[var(--text-2)]"
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <nav className="fixed right-0 top-0 flex h-full w-[min(100vw-2.5rem,18rem)] max-w-full flex-col border-l border-[var(--border-default)] bg-[var(--surface-1)] p-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpen(false)} className="text-[var(--text-3)]" aria-label="Close menu">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden>
