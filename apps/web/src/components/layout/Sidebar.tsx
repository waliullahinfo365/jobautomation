...
import { useAuthSession, useAdvancedUi } from "@/context/AuthSessionContext";
import { getSidebarNavItems, isNavItemActive, shouldUseCustomerNav } from "@/config/navigation";
...
export function Sidebar() {
  ...
  const navItems = getSidebarNavItems(productRole);
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={navItemKey(item)}
                href={item.href}
                className={cn("jf-nav-item", isActive && "is-active")}
              >
                <Icon size={16} />
