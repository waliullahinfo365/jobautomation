...
export function UploadDocumentModal({ open, onClose, onSubmit, loading }: Props) {
  ...
  const jobOptions = [{ label: "No related job", value: "" }, ...jobs.map((j) => ({ label: `${j.company} — ${j.position}`, value: getResourceId(j) }))];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="fixed inset-0 bg-black/60 backdrop-blur-[2px]" onClick={() => !loading && onClose()} aria-hidden />
      <div className="relative z-50 w-full max-w-lg rounded-[var(--r-lg)] border border-[var(--border-default)] bg-[var(--surface-2)] p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-[var(--text-1)]">Upload document</h2>
        <p className="mt-1 text-sm text-[var(--text-3)]">Create a document record for your pipeline.</p>
        <form onSubmit={(e) => void handleSubmit(e)} className="mt-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={type}
              onChange={(e) => setType(e.target.value as UploadPayload["type"])}
              options={[
                { label: "CV", value: "CV" },
