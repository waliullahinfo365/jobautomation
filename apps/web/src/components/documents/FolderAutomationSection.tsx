    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" variant="secondary" disabled={provisionDisabled} onClick={() => void onProvisionJobFolder?.()}>
          {t("documents.folderAutomation.provisionJobFolder")}
        </Button>
      </div>
      <FolderActivityTable records={activity} />
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("documents.folderAutomation.settingsTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <Row label={t("documents.folderAutomation.autoCreateFolder")}>
          ...
          <div>
            <p className="text-xs text-[var(--text-3)]">{t("documents.folderAutomation.namingConvention")}</p>
