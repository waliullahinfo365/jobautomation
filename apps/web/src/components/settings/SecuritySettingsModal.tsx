...
export function SecuritySettingsModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onSave?.(settings);
      showInfo("Security settings are saved locally for this demo. Backend security enforcement will be connected next.");
      onClose();
    } finally {
      setIsSaving(false);
    }
      ...
      <div className="space-y-4">
        <div className="space-y-3">
          <ToggleRow
            ...
            description="Require 2FA code on login"
        ...
        <div className="bg-[var(--surface-3)] rounded-lg p-3 text-xs text-[var(--text-3)]">
          <p>
            These settings are saved locally for demonstration purposes. Production implementation will enforce these policies on the backend.
          </p>
        </div>
