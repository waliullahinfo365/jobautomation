...
import { ArrowRight, Sparkles, CheckCircle2, TrendingUp, Zap, Brain, Mail } from "lucide-react";
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {particles.map(({ id, x, y, size, duration, delay, opacity }) => (
        <motion.div
          ...
          animate={{ y: [0, -30, 10, -20, 0], x: [0, 15, -10, 8, 0], opacity: [opacity, opacity * 1.8, opacity * 0.6, opacity * 1.4, opacity], scale: [1, 1.3, 0.8, 1.1, 1] }}
...
function GridBackground() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      />
      <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-full max-w-3xl h-[600px]" style={{ background: `radial-gradient(ellipse at center, var(--lp-hero-glow) 0%, var(--lp-hero-glow2) 35%, transparent 70%)` }} />
    </div>
  );
}
...
function BeamEffect() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <motion.div className="absolute h-px w-[200px] sm:w-[300px]" style={{ background: "linear-gradient(90deg, transparent, rgba(99,102,241,0.6), rgba(168,85,247,0.6), transparent)", top: "35%" }} animate={{ x: ["-30vw", "130vw"] }} transition={{ duration: 6, repeat: Infinity, ease: "linear", repeatDelay: 4 }} />
      <motion.div className="absolute h-px w-[150px] sm:w-[200px]" style={{ background: "linear-gradient(90deg, transparent, rgba(6,182,212,0.5), transparent)", top: "65%" }} animate={{ x: ["130vw", "-30vw"] }} transition={{ duration: 8, repeat: Infinity, ease: "linear", repeatDelay: 2, delay: 3 }} />
    </div>
  );
}
...
function CursorGlow() {
  ...
  return (
    <motion.div
      ...
      style={{ x, y, width: 400, height: 400, translateX: "-50%", translateY: "-50%", background: isDark ? "radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 70%)" : "radial-gradient(circle, rgba(99,102,241,0.04) 0%, transparent 70%)", borderRadius: "50%" }}
    />
  );
}

// ── Mobile app preview (simple, readable on small screens) ────────────────────
            ...
            <div>
              ...
              <div className="mt-1.5 flex gap-1 flex-wrap">
                {["Google", "Airbnb"].map((co) => (
                  <span key={co} className="text-[9px] px-1.5 py-0.5 rounded-full text-green-600 dark:text-green-400" style={{ background: "rgba(34,197,94,0.15)" }}>{co}</span>
                ))}
              </div>
            </div>
          </div>
...
// ── Desktop mockup (richer, shown on md+) ────────────────────────────────────
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-5 py-3" style={{ background: "var(--lp-card2)", borderBottom: "1px solid var(--lp-bd)" }}>
        ...
        <div className="ml-4 flex-1 max-w-xs h-6 rounded-md flex items-center px-3 gap-2" style={{ background: "var(--lp-card)" }}>
          <div className="w-2 h-2 rounded-full bg-green-400/60" />
          <span className="text-[10px]" style={{ color: "var(--lp-t4)" }}>app.newjobguru.com/dashboard</span>
        </div>
      </div>
      <div className="flex min-h-[300px]">
        <div className="hidden lg:flex flex-col w-32 p-3 gap-0.5 shrink-0" style={{ background: "var(--lp-card3)", borderRight: "1px solid var(--lp-bd3)" }}>
          {["Dashboard", "My Jobs", "Applications", "Documents", "Insights"].map((item, i) => (
            <div key={item} className="text-[11px] px-2.5 py-1.5 rounded-lg cursor-default" style={i === 0 ? { background: "rgba(59,130,246,0.15)", color: "#60a5fa", fontWeight: 500 } : { color: "var(--lp-t4)" }}>{item}</div>
          ))}
        </div>
        <div className="flex-1 p-4 space-y-3 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full border" style={{ background: "rgba(34,197,94,0.10)", color: "#4ade80", borderColor: "rgba(34,197,94,0.25)" }}>
              <div className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />AI Active
            </div>
          </div>
          <div className="grid grid-cols-4 gap-2">
            {statCards.map(({ label, value, change, c }) => (
              <div key={label} className="rounded-xl p-2.5" style={{ background: "var(--lp-card)", border: "1px solid var(--lp-bd)" }}>
                <div className="text-[9px]" style={{ color: "var(--lp-t4)" }}>{label}</div>
                <div className="text-xl font-bold tabular-nums" style={{ color: "var(--lp-t1)" }}>{value}</div>
                <div className="text-[9px]" style={{ color: c }}>{change}</div>
              </div>
            ))}
          </div>
          <div className="rounded-xl overflow-hidden" style={{ background: "var(--lp-card)", border: "1px solid var(--lp-bd)" }}>
            <div className="flex items-center justify-between px-3 py-2" style={{ borderBottom: "1px solid var(--lp-bd3)" }}>
              ...
              <span className="text-[9px] text-blue-500">View all →</span>
            </div>
            ...
            <div className="rounded-xl p-3" style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.15)" }}>
              ...
              <div className="flex flex-wrap gap-1">
                {["Google", "Airbnb", "Figma"].map((co) => (
                  <span key={co} className="text-[8px] px-1.5 py-0.5 rounded-full text-green-600 dark:text-green-400" style={{ background: "rgba(34,197,94,0.12)" }}>{co}</span>
                ))}
              </div>
            </div>
          </div>
...
export function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-start overflow-hidden pt-20 sm:pt-24 lg:pt-28 pb-12 sm:pb-16 lg:pb-20">
      <GridBackground />
      <Particles />
      <BeamEffect />
      <CursorGlow />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 rounded-full border border-blue-500/30 bg-blue-500/10 mb-6 sm:mb-8 cursor-default"
        >
          <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }}>
            <Sparkles size={11} className="text-blue-500" />
          </motion.div>
          <span className="text-xs sm:text-sm text-blue-500 font-medium">AI-Powered Job Application Automation</span>
          <span className="hidden sm:inline-flex px-1.5 py-0.5 rounded-full bg-blue-500/20 text-[10px] text-blue-500 font-bold border border-blue-500/30">NEW</span>
        </motion.div>

        {/* Headline */}
        <h1 className="text-[2rem] leading-[1.1] sm:text-5xl lg:text-[72px] font-bold lg:leading-[1.06] tracking-tight mb-4 sm:mb-6">
          <div className="flex flex-wrap justify-center gap-x-3 sm:gap-x-4 gap-y-1">
            {["Your", "AI-Powered", "Job", "Search", "Command", "Center"].map((word, i) => (
              <motion.span
                key={word}
                initial={{ opacity: 0, y: 30, filter: "blur(8px)" }}
                animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
                transition={{ delay: 0.1 + i * 0.07, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                className={i >= 2 ? "bg-clip-text text-transparent" : undefined}
                style={i >= 2 ? { backgroundImage: "linear-gradient(135deg, #60a5fa 0%, #a78bfa 50%, #22d3ee 100%)" } : { color: "var(--lp-t1)" }}
              >
                {word}
              </motion.span>
            ))}
        ...
        >
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto">
            <Link
              href="/login"
              className="group flex items-center justify-center gap-2 w-full sm:w-auto px-7 py-3.5 rounded-full text-white font-semibold text-base"
              style={{ background: "linear-gradient(135deg,#2563eb,#7c3aed)", boxShadow: "0 0 40px rgba(99,102,241,0.35),0 4px 20px rgba(0,0,0,0.25)" }}
            >
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto">
            <Link
              href="/login"
              className="flex items-center justify-center gap-2 w-full sm:w-auto px-7 py-3.5 rounded-full font-semibold text-base"
        ...
        >
          {[
            { icon: TrendingUp, iconClass: "text-green-500", stat: "10,000+", label: "tracked" },
            { icon: Zap, iconClass: "text-blue-500", stat: "95%", label: "automation success" },
            { icon: CheckCircle2, iconClass: "text-purple-500", stat: "4x faster", label: "job search" },
          ].map(({ icon: Icon, iconClass, stat, label }) => (
            <div key={stat} className="flex items-center gap-1.5" style={{ color: "var(--lp-t3)" }}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-xs mb-8 sm:mb-12" style={{ color: "var(--lp-t5)" }}>
          Trusted by job seekers worldwide · Privacy first · Cancel anytime
        </motion.p>

        {/* Mockup - different for mobile vs desktop */}
        <div className="relative w-full max-w-5xl mx-auto">
          <div className="absolute inset-0 -z-10 blur-3xl opacity-30" style={{ background: "radial-gradient(ellipse at 50% 70%, rgba(59,130,246,0.3) 0%, rgba(139,92,246,0.15) 50%, transparent 80%)" }} />

          {/* Mobile: simplified clean preview */}
          <div className="block md:hidden">
            <MobileAppPreview />
          </div>

          {/* Desktop: full mockup with floating cards */}
          <div className="hidden md:block relative">
            <DesktopMockup />

            {/* Floating cards */}
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.1, duration: 0.5 }} className="absolute -left-2 lg:-left-10 top-1/4 rounded-2xl p-3.5 w-44 hidden lg:block" style={{ background: "var(--lp-bg2)", border: "1px solid var(--lp-bd2)", boxShadow: "var(--lp-shadow)", backdropFilter: "blur(20px)" }}>
              <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
                ...
                <div className="mt-2 flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-purple-400" /><span className="text-[9px] text-purple-500">Calendar synced</span></div>
              </motion.div>
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.3, duration: 0.5 }} className="absolute -right-2 lg:-right-10 top-1/3 rounded-2xl p-3.5 w-48 hidden lg:block" style={{ background: "var(--lp-bg2)", border: "1px solid var(--lp-bd2)", boxShadow: "var(--lp-shadow)", backdropFilter: "blur(20px)" }}>
              <motion.div animate={{ y: [0, 6, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 0.8 }}>
                ...
                <div className="mt-2 flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-blue-400" /><span className="text-[9px] text-blue-500">94% match score</span></div>
              </motion.div>
            </motion.div>
          </div>
        </div>
