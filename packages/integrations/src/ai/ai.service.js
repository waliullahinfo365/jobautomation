...
export async function extractJobFromEmail(payload) {
    ...
    const locationKw = LOCATION_KEYWORDS.find((keyword) => payload.bodyText.toLowerCase().includes(keyword));
...
function shouldUseStub(config) {
    return config.provider === "Stub" || config.fallbackToStub || !config.apiKeyDecrypted;
}
function usageFromLengths(inputLen, outputLen) {
...
export async function runAiExtraction(input) {
    ...
    const provider = usedStub ? "Stub" : input.config.provider;
